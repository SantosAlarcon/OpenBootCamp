...
import { alreadyParticipated, kataRatedbyUser, solveKata } from "../domain/orm/Kata.orm";
...
kataRouter.route("/solve")
  .post(jsonParser, verifyToken, async (req: Request, res: Response) => {
    ...
    if (id && solution) {
      ...
      const isAlreadyParticipated: any = await alreadyParticipated(id, idUsuarioActual);

      LogInfo(`${isAlreadyParticipated}`);
