...
export class UsersController implements IUserController {
     ...
     */
    @Get("/katas")
    public async getKatasByUser(@Query() page: number, @Query() limit: number, @Query() id: string): Promise<any> {
        let response: any;

        if (id) {
            LogSuccess(`[/api/users/katas] Obteniendo las katas del usuario con el ID ${id}...`);
            await getKatasFromUser(page, limit, id).then((katas: IKata[]) => {
                LogSuccess(`[/api/users/katas] Se han encontrado las katas del usuario ${id}...`);
                response = katas;
            });
        } else {
            LogWarning("[/api/users/katas] No se ha pasado el ID de usuario...");
            response = {
