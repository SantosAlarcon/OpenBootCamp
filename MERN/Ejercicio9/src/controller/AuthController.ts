...
export class AuthController implements IAuthController {
  ...
  public async registerUser(@Body() user: IUser): Promise<any> {
    ...
    if (user) {
      ...
      LogWarning(
        `[/api/auth/register] Tratando de registrar usuario sin la información del mismo`
      );
      response = {
        message:
          "Es obligatoria la información del usuario para poder registrarlo.",
        token: "No válido",
      };
    }
  ...
  public async loginUser(@Body() auth: IAuth): Promise<AuthResponse> {
    ...
    if (auth) {
      LogSuccess(
        `[/api/auth/login] Iniciando sesión del usuario ${auth.email}`
      );
      ...
      LogWarning(
        `[/api/auth/login] Se requiere introducir el email y la contraseña`
      );
      response = {
        message:
          "Es obligatorio proporcionar el email y la contraseña para iniciar sesión.",
  ...
  public async userData(@Query() id: string): Promise<any> {
    ...
    if (id) {
      LogSuccess(
        `[/api/auth/me] Obteniendo datos del usuario con el ID ${id}.`
      );
